<?php
    ...
    $application_name=isset($_POST['APP']) ? $_POST['APP'] : null;
...
<head>
	<title>Listes des serveurs d'une application (triés par OS)</title>
...
<body>
	...
	<div style='text-align:center;'>
		<h4 class="jumbotron">Listes des serveurs d'une application (triés par OS)</h4><br />
    </div>
    <div>
        <form id="valid" name="valid_app" class="form-group form-group-lg" method="POST" enctype="multipart/form-data"	action="search_servers_by_appli.php"> 
            <input list='list_app' name='APP' id='APP' width='auto' class='input-lg' onchange='document.getElementById("valid").submit()' value='<?php echo $application_name?>' onclick="if(this.value!='')this.value=''">
                <datalist id="list_app">
                <option valeur="" ></option>
                   <?php
                        $queryOSNAME="select application from cmdb.application";
                        if ($stmt = $con->prepare($queryOSNAME))
                        {
                            $stmt->execute();
                            while ($resulttable = $stmt->fetch())
                            {
                                if ($application_name == strtoupper($resulttable[0]))
                                {
